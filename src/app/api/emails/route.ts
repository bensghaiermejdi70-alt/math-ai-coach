...
export async function POST(req: NextRequest) {
  try {
    ...
    // Vérifier clé secrète (appels serveur seulement)
    ...
    switch (type) {
      ...
      case 'confirmation_abonnement':
        html = emailConfirmationAbonnement(data.nom, data.plan, data.dateExpiration, data.montant)
        subject = '✅ Abonnement Bac.AI confirmé'
        break
      case 'expiration_bientot':
        html = emailExpirationBientot(data.nom, data.dateExpiration, data.plan)
