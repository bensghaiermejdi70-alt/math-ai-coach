...
import { getMatiereFrance, FicheChapitreFrance } from '@/lib/fiches-data-france'

export default function FicheMatiereFrancePage() {
  const params = useParams<{ matiere: string }>()
  const matiere = getMatiereFrance(params.matiere)

  const [selectedChapitre, setSelectedChapitre] = useState<FicheChapitreFrance | null>(null)
  const printFrameRef = useRef<HTMLIFrameElement>(null)

  const pdfUrl = matiere && selectedChapitre ? `/fiches/france/${matiere.slug}/${selectedChapitre.file}` : null

  // Regroupement par unité si présent (ex: futurs thèmes de Physique-Chimie), sinon liste plate
  const groupes = useMemo(() => {
    if (!matiere) return []
    const map = new Map<string, FicheChapitreFrance[]>()
    ...
    return (
      <>
        ...
        <main style={{ paddingTop: 140, textAlign: 'center', minHeight: '100vh' }}>
          ...
          <Link href="/fiches-revision/france" className="btn btn-secondary" style={{ marginTop: 16, display: 'inline-block' }}>← Retour</Link>
        </main>
        <Footer />
      </>
    )
      ...
      <>
        ...
        <main style={{ position: 'relative', zIndex: 1, paddingTop: 80, minHeight: '100vh' }}>
          <div className="container" style={{ paddingTop: 100, paddingBottom: 100, maxWidth: 700, textAlign: 'center' }}>
            ...
            <Link href="/fiches-revision/france" className="btn btn-primary">← Voir les autres matières</Link>
          </div>
        </main>
        <Footer />
      </>
    ...
    <>
      ...
      <main style={{ position: 'relative', zIndex: 1, paddingTop: 80, minHeight: '100vh' }}>
        <div className="container" style={{ paddingTop: 48, paddingBottom: 80, maxWidth: 1100 }}>
          ...
          <Link href="/fiches-revision/france" style={{ color: 'var(--text2)', fontSize: 13, textDecoration: 'none' }}>← Toutes les matières</Link>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, margin: '18px 0 36px' }}>
            ...
            <div>
              ...
              <p style={{ color: 'var(--text2)', fontSize: 13, margin: '4px 0 0' }}>{matiere.chapitres.length} chapitres · Bac France · Terminale</p>
            </div>
          </div>
            ...
            {selectedChapitre && (
              <div>
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
                  <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>{selectedChapitre.titre}</span>
