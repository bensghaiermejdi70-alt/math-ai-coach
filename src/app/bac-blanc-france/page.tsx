...
function sanitizeExpr(expr: string): string {
  let e = expr
    // Notations françaises : f(x)=... g(x)=... h(x)=... → extraire la partie droite
    .replace(/^[a-zA-Z_]\w*\s*\([^)]*\)\s*=\s*/, '')
    // Suites u_n, v_n → remplacer n par x pour le tracé
    .replace(/u_n|v_n|w_n/g, 'x')
    // e^(...) → Math.exp(...)
    .replace(/e\^\(([^)]+)\)/g, (_,a) => 'Math.exp(' + a + ')')
    ...
    // Valeur absolue |x| → Math.abs(x)
    .replace(/\|([^|]+)\|/g,(_,a)=>'Math.abs('+a+')')
  ...
  // e seul (constante d'Euler) mais pas dans Math.exp etc
