...
// ══════════════════════════════════════════════════════════════════════
// PHYSIQUE-CHIMIE TUNISIE — Sciences Exp. — PAGE SLUG
...
const NAV_ORDER = ['dipole-rc', 'dipole-rl', 'oscillations-lc', 'oscillations-mecaniques', 'ondes-mecaniques', 'ondes-lumineuses-exp', 'nucleaire-exp', 'cinetique-exp', 'equilibres-exp', 'acide-base-exp', 'electrochimie-exp', 'organique-exp']
const TITRES_NAV: Record<string,string> = {
  'dipole-rc': 'CH.1 — Dipôle RC',
  'dipole-rl': 'CH.2 — Dipôle RL',
  'oscillations-lc': 'CH.3 — Oscillations LC',
  'oscillations-mecaniques': 'CH.4 — Oscillateurs mécaniques',
  'ondes-mecaniques': 'CH.5 — Ondes mécaniques',
  'ondes-lumineuses-exp': 'CH.6 — Ondes lumineuses',
  'nucleaire-exp': 'CH.7 — Réactions nucléaires',
  'cinetique-exp': 'CH.1 — Cinétique chimique',
  'equilibres-exp': 'CH.2 — Équilibres chimiques',
  'acide-base-exp': 'CH.3 — Acides et bases',
  'electrochimie-exp': 'CH.4 — Électrochimie',
  'organique-exp': 'CH.5 — Chimie organique',
}
const SEC_COLORS: Record<string,string> = {
  'dipole-rc':'#4f6ef7',
  'dipole-rl':'#8b5cf6',
  'oscillations-lc':'#06b6d4',
  'oscillations-mecaniques':'#10b981',
  'ondes-mecaniques':'#f59e0b',
  'ondes-lumineuses-exp':'#14b8a6',
  'nucleaire-exp':'#ef4444',
  'cinetique-exp':'#f59e0b',
  'equilibres-exp':'#8b5cf6',
  'acide-base-exp':'#10b981',
  'electrochimie-exp':'#ef4444',
  'organique-exp':'#06b6d4',
}
...
'dipole-rc': {
  id:'dipole-rc', emoji:'🔌', badge:'Physique T1', color:'#4f6ef7',
  titre:"Dipôle RC",
...
'dipole-rl': {
  id:'dipole-rl', emoji:'🌀', badge:'Physique T1', color:'#8b5cf6',
  titre:"Dipôle RL",
...
'oscillations-lc': {
  id:'oscillations-lc', emoji:'〰️', badge:'Physique T1', color:'#06b6d4',
...
'oscillations-mecaniques': {
  id:'oscillations-mecaniques', emoji:'🔄', badge:'Physique T2', color:'#10b981',
...
'ondes-mecaniques': {
  id:'ondes-mecaniques', emoji:'🌊', badge:'Physique T2', color:'#f59e0b',
...
'ondes-lumineuses-exp': {
  id:'ondes-lumineuses-exp', emoji:'🌈', badge:'Physique T3', color:'#14b8a6',
...
'nucleaire-exp': {
  id:'nucleaire-exp', emoji:'☢️', badge:'Physique T3', color:'#ef4444',
...
'cinetique-exp': {
  id:'cinetique-exp', emoji:'⏱️', badge:'Chimie T1', color:'#f59e0b',
...
'equilibres-exp': {
  id:'equilibres-exp', emoji:'⚖️', badge:'Chimie T1', color:'#8b5cf6',
...
'acide-base-exp': {
  id:'acide-base-exp', emoji:'🧪', badge:'Chimie T2', color:'#10b981',
...
'electrochimie-exp': {
  id:'electrochimie-exp', emoji:'🔋', badge:'Chimie T3', color:'#ef4444',
  titre:"Électrochimie",
...
'organique-exp': {
  id:'organique-exp', emoji:'🔬', badge:'Chimie T3', color:'#06b6d4',
...
// ─── Composant principal ─────────────────────────────────────────────
export default function PhysiqueSciencesExpSlugPage() {
      ...
      <>
        ...
        <main style={{ paddingTop:120, paddingBottom:80 }}>
          <div className="container" style={{ textAlign:'center' }}>
            ...
            <Link href="/bac/physique/sciences-exp" className="btn btn-primary">← Retour Sciences Exp.</Link>
          </div>
        </main>
        <Footer />
      </>
      ...
      <main style={{ position:'relative', zIndex:1, paddingTop:80 }}>
        <div className="container" style={{ paddingTop:32, paddingBottom:80 }}>
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', fontSize:12, color:'var(--muted)', marginBottom:24, flexWrap:'wrap' }}>
            ...
            <span>›</span>
            <Link href="/bac/physique/sciences-exp" style={{ color:'var(--muted)', textDecoration:'none' }}>Sciences Exp.</Link>
            <span>›</span>
            ...
            <div>
              ...
              <div style={{ background:`${secColor}10`, border:`1px solid ${secColor}28`, borderRadius:18, padding:'24px 28px', marginBottom:24 }}>
                ...
                <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                  <Link href={`/chat?q=${encodeURIComponent('Explique-moi '+chapter.titre+' Bac Tunisie Sciences Exp.')}`} className="btn btn-primary" style={{ fontSize:12 }}>
                                  ...
                                  {bloc.exercices.map(ex => (
                                    <div key={ex.id} style={{ border:'1px solid var(--border)', borderRadius:10, overflow:'hidden' }}>
                                      ...
                                      <div style={{ borderTop:'1px solid var(--border)', padding:'8px 16px', display:'flex', gap:8, flexWrap:'wrap' }}>
                                        <Link href={`/solve?q=${encodeURIComponent('Bac Tunisie PC Sciences Exp. — '+ex.enonce)}`} className="btn btn-primary" style={{ fontSize:11, padding:'5px 12px' }}>
                                          🧮 Résoudre avec IA
                                        </Link>
            ...
            <aside style={{ position:'sticky', top:88 }}>
              ...
              <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden', marginBottom:12 }}>
                <div style={{ padding:'10px 14px', borderBottom:'1px solid var(--border)', fontSize:11, color:'var(--muted)', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em' }}>
                  🔬 Sciences Exp. · {NAV_ORDER.length} chapitres
                </div>
                {/* Physique */}
                ...
                {NAV_ORDER.filter(s => !['cinetique-exp', 'equilibres-exp', 'acide-base-exp', 'electrochimie-exp', 'organique-exp'].includes(s)).map((s,i) => (
                ...
                {['cinetique-exp', 'equilibres-exp', 'acide-base-exp', 'electrochimie-exp', 'organique-exp'].map((s,i) => (
                  <Link key={s} href={`/bac/physique/sciences-exp/${s}`} style={{ textDecoration:'none' }}>
              ...
              <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'13px' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  <Link href={`/chat?q=${encodeURIComponent('Explique-moi '+chapter.titre+' Bac Tunisie Sciences Exp.')}`} className="btn btn-primary" style={{ textAlign:'center', fontSize:12 }}>
                    🤖 Chat IA
                  </Link>
                  ...
                  <Link href="/bac/physique/sciences-exp" className="btn btn-secondary" style={{ textAlign:'center', fontSize:12 }}>↩ Sciences Exp.</Link>
