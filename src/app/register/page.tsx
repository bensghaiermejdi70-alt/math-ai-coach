...
export default function RegisterPage() {
  ...
  async function handleGoogle() {
    setGoogleL(true)
    // Même flux que la page login : OAuth Google → callback
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:'20px', position:'relative', zIndex:1 }}>
      ...
      <div style={{ width:'100%', maxWidth: isAdminEmail ? 440 : 500, position:'relative', zIndex:1, animation:'fadeInUp 0.6s ease both', transition:'max-width 0.3s' }}>
        ...
        <div className="card-glass" style={{ padding:'36px 40px', borderRadius:20 }}>
          ...
          <button type="button" onClick={handleGoogle} disabled={googleL}
            style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'center', gap:10, padding:'12px', borderRadius:12, border:'1px solid rgba(255,255,255,0.15)', background:'rgba(255,255,255,0.06)', color:'white', fontSize:14, fontWeight:600, cursor: googleL ? 'not-allowed' : 'pointer', marginBottom:18, transition:'all 0.2s', opacity: googleL ? 0.7 : 1 }}
            onMouseEnter={e => { if(!googleL) e.currentTarget.style.background='rgba(255,255,255,0.1)' }}
            ...
            {googleL ? 'Redirection...' : "S'inscrire avec Google"}
          </button>
